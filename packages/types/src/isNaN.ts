 ...
 * ### 全局的 `isNaN`
*
 * 全局的 `isNaN` 方法可以判断任意的值，并存在类型隐式转换的行为，它会将 `value` 隐式转换为 `Number` (`Number(value)`)。
 ...
 * ### `Number.isNaN`
 *
 * `Number.isNaN` 并不会对 `value` 进行隐式转换 (`Number(value)`)。如果 `value` 不是一个数字，则直接返回 `false`。
 * 该方法其实也可以用来判断字符串，写成 `isNaN(+value)` 即可。如果是判断其他值，则可以写成 `isNaN(Number(value))`。
 *
 * ### 返回 `NaN` 的五种情况
 *
