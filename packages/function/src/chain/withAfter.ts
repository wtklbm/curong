/**
 * 创建一个新的函数，该函数在调用目标函数后，会将目标函数的返回值作为第一个参数传递给后置函数，并返回目标函数的结果
 *
 ...
 *
 * ```typescript
 * const log = withAfter(data => console.log(`结果: ${data}`));
 *
 ...
 * ```
 */
export default function withAfter<R, T, P extends unknown[]>(
    ...
    return <A extends unknown[]>(
        ...
        const ret = targetFunc.apply(targetFunc, targetArgs);
        afterFunc.call(afterFunc, ret, ...afterArgs);
        return ret;
    };
}
